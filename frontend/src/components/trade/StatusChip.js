...
function StatusChip(_a) {
    var label = _a.label, state = _a.state, compact = _a.compact;
    var s = stateStyles[state];
    return (<span className={"inline-flex max-w-full items-center truncate rounded-full border border-white/[0.06] font-semibold tracking-wide ring-1 ".concat(s.ring, " ").concat(s.bg, " ").concat(s.text, " ").concat(s.glow, " ").concat(compact ? 'px-[5px] py-[1px] text-[8px] leading-none' : 'px-2 py-0.5 text-[10px]')}>
      {label}
    </span>);
}
