...
exports.TradeReviewHeader = TradeReviewHeader;
...
function TradeReviewHeader(_a) {
    var pair = _a.pair, direction = _a.direction, setupType = _a.setupType, score = _a.score, state = _a.state, sourceEngine = _a.sourceEngine;
    var tier = score != null && Number.isFinite(score) ? (0, botSystem_1.getScoreTier)(score) : null;
    return (<framer_motion_1.motion.header initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 backdrop-blur-sm">
      ...
      <div className="mt-2 flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          {sourceEngine ? (<p className="mt-0.5 text-[11px] text-zinc-500">
              Engine · <span className="text-zinc-400">{sourceEngine}</span>
            </p>) : null}
        </div>
        <span className={"shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ".concat(statePillClass(state))}>
          {state}
        </span>
      </div>
