...
exports.overextendedScenario5m = exports.pullbackScenario5m = exports.breakoutScenario5m = void 0;
var START_TS = Date.UTC(2026, 0, 5, 12, 0, 0);
var STEP_MS = 5 * 60 * 1000;
function c(i, open, high, low, close, volume) {
    return {
...
function prependWarmup(seed, opts) {
    var warm = [];
    var price = opts.startPrice;
    for (var i = 0; i < opts.count; i += 1) {
        var wobble = Math.sin(i / 3) * opts.spread * 0.35;
        var close_1 = Math.max(0.0001, price + opts.drift + wobble);
        var high = Math.max(price, close_1) + opts.spread;
        var low = Math.min(price, close_1) - opts.spread;
        var vol = opts.baseVolume * (0.92 + (i % 6) * 0.03);
        warm.push(c(i, price, high, low, close_1, vol));
        price = close_1;
    }
    var shifted = seed.map(function (bar, idx) {
        return c(opts.count + idx, bar.open + (price - opts.startPrice), bar.high + (price - opts.startPrice), bar.low + (price - opts.startPrice), bar.close + (price - opts.startPrice), bar.volume);
    });
    return __spreadArray(__spreadArray([], warm, true), shifted, true);
}
var breakoutCore = [
...
exports.breakoutScenario5m = prependWarmup(breakoutCore, {
    count: 48,
    startPrice: 96,
    drift: 0.16,
    spread: 0.55,
...
exports.pullbackScenario5m = prependWarmup(pullbackCore, {
    count: 20,
    startPrice: 192,
    drift: 0.28,
    spread: 0.9,
...
exports.overextendedScenario5m = prependWarmup(overextendedCore, {
    count: 48,
    startPrice: 45,
    drift: 0.18,
    spread: 0.45,
