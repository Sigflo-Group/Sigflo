...
exports.sanitizeUserFacingHttpErrorMessage = sanitizeUserFacingHttpErrorMessage;
/**
 * Collapse CDN/HTML error pages into a short hint for UI and thrown Error messages.
 */
function sanitizeUserFacingHttpErrorMessage(message) {
    var t = message.trim();
    if (t.length === 0)
        return t;
    if (/<\s*!doctype/i.test(t) ||
        ...
        return ('HTTP 403 — something returned an HTML error page (e.g. CloudFront) instead of JSON. ' +
