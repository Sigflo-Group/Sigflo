...
exports.default = OnboardingTradingStyleScreen;
...
function OnboardingTradingStyleScreen() {
    var navigate = (0, react_router_dom_1.useNavigate)();
    var _a = (0, AuthContext_1.useAuth)(), user = _a.user, authMode = _a.authMode;
    var pick = (0, react_1.useCallback)(function (choice) {
        (0, tradingStyleOnboarding_1.markTradingStyleOnboarded)(choice);
    ...
    if ((0, tradingStyleOnboarding_1.isTradingStyleOnboarded)()) {
        return (0, exchangeConnectOnboarding_1.isExchangeConnectOnboardingSeen)() ? (<react_router_dom_1.Navigate to={(0, appRoutes_1.getFeedRoute)()} replace/>) : (<react_router_dom_1.Navigate to="/onboarding/connect" replace/>);
    }
    return (<div className="min-h-[100dvh] bg-[#0F1115] px-5 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(2.5rem,env(safe-area-inset-top))]">
      ...
      <div className="relative mx-auto max-w-sm">
        <div className="mb-8 flex flex-col items-center text-center">
          <SigfloLogo_1.SigfloLogo size={44} glowing className="mb-5"/>
        ...
        <div className="space-y-3">
          {OPTIONS.map(function (_a) {
            var choice = _a.choice, title = _a.title, bot = _a.bot, blurb = _a.blurb;
            return (<button key={choice} type="button" onClick={function () { return pick(choice); }} className="flex w-full flex-col items-start rounded-2xl border border-white/[0.08] bg-[#171A20] px-4 py-4 text-left shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)] transition hover:border-[rgba(0,200,120,0.35)] hover:bg-[#1a1e26] active:scale-[0.99]">
