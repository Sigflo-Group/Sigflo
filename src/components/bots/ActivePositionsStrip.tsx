...
type ActivePositionsStripProps = {
  positions: Position[];
  onSelectPosition?: (pair: string) => void;
};

export default function ActivePositionsStrip({ positions, onSelectPosition }: ActivePositionsStripProps) {
  if (positions.length === 0) return null;

  return (
    <div className="rounded-2xl border border-[#00ffc8]/16 bg-[rgba(0,255,200,0.04)] px-3 py-2.5 backdrop-blur-md">
      ...
      <div className="-mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-0.5 [scrollbar-width:thin]">
        {positions.map((p) => {
          ...
          const pnlUsd =
            p.unrealizedPnl >= 0
              ? `+$${formatQuoteNumber(Math.abs(p.unrealizedPnl))}`
              : `−$${formatQuoteNumber(Math.abs(p.unrealizedPnl))}`;
          const roe =
