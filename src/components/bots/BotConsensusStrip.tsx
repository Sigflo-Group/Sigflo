import { botPersonalityConsensusRows } from '@/lib/bots';

/** Lightweight cross-agent stance row — trust / comparison, not execution. */
export function BotConsensusStrip() {
  const rows = botPersonalityConsensusRows();
  return (
    <section className="rounded-2xl border border-white/[0.06] bg-landing-surface p-3 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]">
      ...
      <div className="mt-2.5 flex flex-col gap-1.5">
        {rows.map(({ name, stance }) => (
          <div
            key={name}
            className="flex items-center justify-between gap-2 rounded-xl border border-white/[0.06] bg-sigflo-elevated px-2.5 py-2"
          >
