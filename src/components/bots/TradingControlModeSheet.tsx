...
const TEAL = '#00C878';
const SHEET_BG = '#171A20';

type TradingControlModeSheetProps = {
  open: boolean;
  onClose: () => void;
  currentMode: TradingControlMode;
  onSelectMode: (mode: TradingControlMode) => void;
};

export function TradingControlModeSheet({
  open,
  onClose,
  currentMode,
  onSelectMode,
}: TradingControlModeSheetProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="fixed inset-0 z-[95] flex flex-col justify-end" role="presentation">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="sigflo-mode-sheet-title"
        className="relative z-[96] flex max-h-[min(78dvh,520px)] flex-col rounded-t-3xl border border-white/[0.08] shadow-[0_-16px_48px_rgba(0,0,0,0.55)]"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3">
          <div className="flex flex-col gap-2.5">
            {TRADING_CONTROL_MODE_ORDER.map((m) => {
              const meta = TRADING_CONTROL_MODE_META[m];
              const active = currentMode === m;
              const autoOptional = m === 'auto' && !TRADING_AUTO_EXECUTION_ACTIVE;
              return (
                <button
                  key={m}
                  type="button"
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {active ? (
                      <span
                        className="shrink-0 rounded-full px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-[#0F1115]"
                  ...
                  <p className={`mt-1.5 text-[13px] font-medium leading-snug ${active ? 'text-[#b8fff0]' : 'text-white/75'}`}>
                    {meta.sheetDescription}
                  </p>
                  <p className="mt-1 text-[11px] leading-relaxed text-sigflo-muted">{meta.sheetSubtext}</p>
                  {autoOptional ? (
                    <p className="mt-2 rounded-lg border border-amber-400/20 bg-amber-500/[0.07] px-2 py-1.5 text-[10px] leading-snug text-amber-100/90">
